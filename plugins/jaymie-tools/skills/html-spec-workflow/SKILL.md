...
2. **给处境，不只给约束**。把产物的寿命、风险容忍度、所处阶段告诉模型，让它自己推导出合适的工程标准，而不是替它列一堆规则。范例（来自 Thariq Shihipar）："This feature might be deleted in a month, don't build anything painful to throw away."（这个功能可能一个月后就删掉，别做任何扔掉会心疼的东西。）
3. **明示信任**。在 prompt 里加一句 "Hey Claude, I trust you here." 不是仪式，是让模型知道它有发挥空间。
4. **让人想看**。HTML 的全部价值就是让人愿意打开、愿意滚动、愿意点击。任何让页面变枯燥的细节都是反指标。
5. **复杂度自证**。每加一个交互、一个组件、一个动画，问一句："它真的让用户决策更快/更好了吗？"
6. **丰盈思维**。token 便宜了，一次性 UI、一次性图表、一次性脚本都值得做。但产物的"长期维护成本"要自觉控制——能扔就扔。
7. **99% / 1%**。绝大多数生成的 token 应该花在规划、界面、沟通上；最终落地代码只是冰山一角。
